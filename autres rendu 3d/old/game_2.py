...
class Wall():
	def __init__(self,u,v,w,text,door,deco,freq,phase):
		...
		if (len(files)>1) or (len(files_d)>1):
			for k in range(max(len(files)-1,len(files_d)-1)):
				...
				self.transparency .append( np.flip(
					(((IM - np.array([255, 0, 255])) == 0).all(-1)) + (((IM - np.array([0, 255, 255])) == 0).all(-1)),
					(1, 0)))
		...
		if self.ID in light_wall.keys():
			Y0 = np.array([source_pos(i)  for i in light_wall[self.ID]])


			for c,Im in enumerate(self.wall_im):
				Ws=Im.shape
				Xw,Yw=np.indices((Ws[0],Ws[1]))
				Xw=Xw*1/Ws[0]
				Yw=Yw*1/Ws[1]
				#print(Y0.shape,self.a[0,0,:],self.b[0,0,:],self.X[0,0,:])
				posw=np.expand_dims(np.expand_dims(Xw,-1)*self.a[0,0,:]+np.expand_dims(Yw,-1)*self.b[0,0,:]+self.X[0,0,:],2)
				POS = np.amin(np.linalg.norm(posw - Y0, axis=-1), axis=-1)
				self.wall_im[c] = np.minimum((0.8 * (np.divide(Im, 0.01 * np.expand_dims((4 * POS) ** 2, -1)) +
									np.divide(Im,0.1 * np.expand_dims((POS),-1))) + 0.2 * Im),255)
	...
	def render(self):
		...
		if np.sum(self.U)<5:
			...
			self.G=np.mod(np.maximum(((1-self.S[:,:,:-1])*self.format).astype(int),0)+int(self.phase*c3*0.5),self.borne)
			colorL=[1,1,1]
			...
			depth=self.S[:,:,-1:]*Ue*(filt)+depth*(1-Ue*(filt))
