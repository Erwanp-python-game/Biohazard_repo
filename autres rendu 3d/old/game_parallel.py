...
class Wall():
	def __init__(self,u,v,w,text,door,deco,freq,phase):
		...
		self.window=0
		if np.sum(((IM-np.array([255,0,255]))==0).all(-1))>0:
			self.window=2
		if np.sum(((IM-np.array([0,255,255]))==0).all(-1))>0:
			self.window = 1

		self.transparency=np.flip((((IM - np.array([255, 0, 255])) == 0).all(-1)) + (((IM - np.array([0, 255, 255])) == 0).all(-1)),(1,0))


		self.wall_im=[np.flip(np.minimum(IM,255),(0,1))]
		self.phase=0
		...
		if (len(files)>1) or (len(files_d)>1):
			for k in range(max(len(files)-1,len(files_d)-1)):
				...
				if np.sum(((IM - np.array([0, 255, 255])) == 0).all(-1)) > 0:
					self.window = 1
				self.wall_im.append(np.flip(np.minimum(IM,255),(0,1)))
...
def create_batch(batch):
	...
	for j in batch:
		...
		if j.text[11:-3] not in ['floor7.', 'floor10.']:
			L_0.append(j)
		else:
			L_1.append(j)
	...
	i = L_0[0]
	A0 = np.expand_dims(i.a, 0)
	B0 = np.expand_dims(i.b, 0)
	X0 = np.expand_dims(i.X, 0)
	borne = np.expand_dims(i.borne, 0)
	color = np.expand_dims(np.array(i.colorL0), 0)
	wall_im = np.expand_dims(np.tile(i.wall_im[0],(1,fmax - i.freq + 1,1)), 0)
	Format = np.expand_dims(i.format, 0)
	source = np.expand_dims(np.array(i.Xsource), 0)
	transpa = np.expand_dims(np.tile(i.transparency,(1,fmax - i.freq + 1)),0)


	for i in (L_0[1:]+L_1):

		A0 = np.concatenate((A0, np.expand_dims(i.a, 0)), 0)
		B0 = np.concatenate((B0, np.expand_dims(i.b, 0)), 0)
		X0 = np.concatenate((X0, np.expand_dims(i.X, 0)), 0)
		borne = np.concatenate((borne, np.expand_dims(i.borne, 0)), 0)
		wall_im = np.concatenate((wall_im, np.expand_dims(np.tile(i.wall_im[0],(1,fmax - i.freq + 1,1)), 0)), 0)
		Format = np.concatenate((Format, np.expand_dims(i.format, 0)), 0)
		source = np.concatenate((source, np.expand_dims(np.array(i.Xsource), 0)), 0)
		color = np.concatenate((color, np.expand_dims(np.array(i.colorL0), 0)), 0)
		transpa=np.concatenate((transpa, np.expand_dims(np.tile(i.transparency,(1,fmax - i.freq + 1)), 0)), 0)



	borne = np.expand_dims(borne, (1,2))
	Format = np.expand_dims(Format, (1,2))

	wall_im = wall_im * np.expand_dims(color,(1,2))
	return A0.astype(np.float32), B0.astype(np.float32), X0.astype(np.float32), borne, wall_im, Format, source.astype(np.float32), np.expand_dims(transpa,-1),len(L_1)

def render_batch(A0, B0, X0, borne, wall_im, Format, source,transpa,water):
	r = A0.shape[0]
	M = np.stack((A0 , B0 , -np.repeat(np.expand_dims(screenV.astype(np.float32),0),r,0)), -1)
	B = -X0+ np.expand_dims(screenP.astype(np.float32),0)
	S = np.linalg.solve(M, B)
	S = S.repeat(2,1).repeat(2,2)
	S[:,:, :, :-1] = (np.roll(S[:,:, :, :-1], 1, axis=2) + np.roll(S[:,:, :, :-1], 1, axis=1) + np.roll(
		S[:,:, :, :-1], -1, axis=1) + np.roll(S[:,:, :, :-1], -1, axis=2)) / 4

	U = np.all(S > 0, axis=-1) & np.all(S[:,:, :, :-1] < 1, axis=-1)
	U=np.expand_dims(U,-1)
	G = np.mod(np.maximum(((1 - S[:,:, :, :-1]) * Format).astype(int), 0) ,borne)


	J=np.expand_dims(np.arange(160),(0,2))
	K=np.expand_dims(np.arange(80),(0,1))
	L=np.expand_dims(np.arange(r),(1,2))

	Ar = wall_im[L,G[L,J,K,0],G[L,J,K,1],:]*U
	transpa=transpa[L,G[L,J,K,0],G[L,J,K,1],:]
	depth=S[:,:, :, -1:]*U*(1-transpa)+((1-U)+transpa)*1e6
	Id=np.argmin(depth,axis=0)

	Ar=Ar[Id[:,:,0],J[0,:,:],K[0,:,:]]
	depth=np.min(depth,axis=0)

	# Xl = (S[:, ::2 , ::2 , 0:1] * A0 + S[:, ::2 , ::2 , 1:2] * B0 + X0).repeat(2,1).repeat(2,2) * U
	# D=np.amin(np.linalg.norm(np.expand_dims(np.moveaxis(source,1,2),(1,2))-np.expand_dims(Xl,-1),axis=-2),axis=-1)



	# D = torch.amin(
	# torch.linalg.norm(torch.moveaxis(source, 1, 2).unsqueeze(1).unsqueeze(1) - Xl.unsqueeze(-1), dim=-2),
	# dim=-1)
	...
	# u = torch.unsqueeze(torch.unsqueeze(torch.tensor([i for i in range(A0.shape[0])]), -1), -1)
		...
		# 	S[:, :, :-1] = (torch.roll(S[:, :, :-1], 1, dims=1) + torch.roll(S[:, :, :-1], 1, dims=2) + torch.roll(
		# 		S[:, :, :-1], -1, dims=2) + torch.roll(S[:, :, :-1], -1, dims=1)) / 4
		# 	AbsS = S[:, :, :, -1]
		# 	AbsS[AbsS <= 0] = 1.e13
		# 	S[:, :, :, -1] = (torch.roll(AbsS, 1, dims=1) + torch.roll(AbsS, 1, dims=2) + torch.roll(AbsS, -1,
		# 																							 dims=2) + torch.roll(
		# 		AbsS, -1, dims=1)) / 4
		#
		# U = torch.all(S > 0, dim=-1) & torch.all(S[:, :, :, :-1] < 1, dim=-1) & torch.all(S[:, :, :, -1:] <= 1e12,
		# 																				  dim=-1)
		...
		# Xl = (S[:, ::2 * K, ::2 * K, 0].unsqueeze(-1) * A0 + S[:, ::2 * K, ::2 * K, 1].unsqueeze(-1) * B0 + X0) * Ue[:,
		# 																										  ::2 * K,
		# 																										  ::2 * K,
		# 																										  :]
		# D = torch.amin(
		...
		# Ar2 = wall_im[I, G[I, torch.arange(160).unsqueeze(1), torch.arange(80).unsqueeze(0), 0],
		# 	  G[I, torch.arange(160).unsqueeze(1), torch.arange(80).unsqueeze(0), 1], :].clone()
		#
		# depth2 = torch.amin(depth, dim=0)
		#
		#
		# return Ar2, depth2.repeat_interleave(2, dim=0).repeat_interleave(2, dim=1), Dl.repeat_interleave(2,
		# 																								 dim=0).repeat_interleave(
		# 	2, dim=1)
	return Ar,depth,10#D[Id[:,:,0],J[0,:,:],K[0,:,:]]
...
while running==1:
	...
	if Sky_view:
		...
		Im=Im*np.expand_dims(((depth<1e6).all(-1)),-1)+LAND*np.expand_dims(((depth>=1e6).all(-1)),-1)
		Im=Im*np.expand_dims(((depth<1e6).all(-1)),-1)+SKY*np.expand_dims(((depth>=1e6).all(-1)),-1)
