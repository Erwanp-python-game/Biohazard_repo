...
screenP0=torch.unsqueeze(torch.tensor(screenP),0).to('cuda')
screenV0=torch.unsqueeze(torch.tensor(screenV),0).to('cuda')
...
class Wall():
	...
	def to_device(self):
		...
		self.borne0=torch.tensor(self.borne).to('cuda')
		self.format0=torch.tensor(self.format).to('cuda')
		self.wall_im0=torch.tensor(np.array(self.wall_im.copy())).to('cuda')
		

		
		
	
	def effect_wall(self):
		self.colorL0=light_modif(self.colorL,level,c3)
		
		# ind=c//(12//len(self.wall_im))
		
		
		self.Xsource=np.full((4,3),-1e3)
		if self.ID in light_wall.keys():
			...
			for c,i in enumerate(X0[:min(len(X0),4)]):
				self.Xsource[c,:]=source_pos(i)
...
def update_screen_gpu():
	global screenP0,screenV0
	screenP0=torch.unsqueeze(torch.tensor(screenP),0).to('cuda')
	screenV0=torch.unsqueeze(torch.tensor(screenV),0).to('cuda')
...
def create_batch(batch):
	global Sky_view
	i=batch[0]
	A0=torch.unsqueeze(i.a0,0)
	B0=torch.unsqueeze(i.b0,0)
	X0=torch.unsqueeze(i.X0,0)
	borne=torch.unsqueeze(i.borne0,0)
	color=torch.unsqueeze(torch.tensor(i.colorL0),0)
	fmax=1
	for j in batch:
		fmax=max(fmax,j.freq)
		j.effect_wall()
		if i.window>0 :
			Sky_view=1

	
	wall_im=torch.unsqueeze(i.wall_im0[0],0).repeat((1,1,fmax-i.freq+1,1))
	Format=torch.unsqueeze(i.format0,0)
	L=0
	
	source=torch.unsqueeze(torch.tensor(i.Xsource),0)

	for i in batch[1:]:
		A0=torch.cat((A0,torch.unsqueeze(i.a0,0)),dim=0)
		B0=torch.cat((B0,torch.unsqueeze(i.b0,0)),dim=0)
		X0=torch.cat((X0,torch.unsqueeze(i.X0,0)),dim=0)
		borne=torch.cat((borne,torch.unsqueeze(i.borne0,0)),dim=0)
		wall_im=torch.cat((wall_im,torch.unsqueeze(i.wall_im0[0],0).repeat((1,1,fmax-i.freq+1,1))),dim=0)
		Format=torch.cat((Format,torch.unsqueeze(i.format0,0)),dim=0)
		source=torch.cat((source,torch.unsqueeze(torch.tensor(i.Xsource),0)),dim=0)
		color=torch.cat((color,torch.unsqueeze(torch.tensor(i.colorL0),0)),dim=0)
		
	for i in range(2):
		borne=torch.unsqueeze(borne,1)
		Format=torch.unsqueeze(Format,1)
	
	wall_im=wall_im*color.to('cuda').unsqueeze(1).unsqueeze(1)
	return A0,B0,X0,borne,wall_im,Format,source.to('cuda')
		
	


def render_batch(A0,B0,X0,borne,wall_im,Format,source):


		#ttt=time.time()
		u=torch.unsqueeze(torch.unsqueeze(torch.tensor([i for i in range(A0.shape[0])]),-1),-1)
		
		M=torch.stack((A0,B0,-screenV0.repeat(A0.shape[0],1,1,1)),axis=-1)

		B=-X0+screenP0.repeat(A0.shape[0],1,1,1)
		S=(torch.linalg.solve(M,B))
		S0=S
		K=1
		for i in range(K):
			S=S.repeat_interleave(2,dim=1).repeat_interleave(2,dim=2)
			S[:,:,:-1]=(torch.roll(S[:,:,:-1],1,dims=1)+torch.roll(S[:,:,:-1],1,dims=2)+torch.roll(S[:,:,:-1],-1,dims=2)+torch.roll(S[:,:,:-1],-1,dims=1))/4
			AbsS=S[:,:,:,-1]
			AbsS[AbsS<=0]=1.e13
			S[:,:,:,-1]=(torch.roll(AbsS,1,dims=1)+torch.roll(AbsS,1,dims=2)+torch.roll(AbsS,-1,dims=2)+torch.roll(AbsS,-1,dims=1))/4
		#print(1000*(time.time()-ttt),'1')
		#ttt=time.time()
		U=torch.all(S>0,dim=-1)&torch.all(S[:,:,:,:-1]<1,dim=-1)&torch.all(S[:,:,:,-1:]<=1e12,dim=-1)
		Ue=torch.unsqueeze(U,-1)
		G=torch.remainder(torch.maximum(((1-S[:,:,:,:-1])*Format),torch.tensor(0)),borne).long()
		depth=S[:,::2*K,::2*K,-1:]*Ue[:,::2*K,::2*K,:]
		print(depth.shape,torch.unsqueeze(torch.all(Ar1[::2*K,::2*K,:]<=0,dim=2),-1).shape)
		Xl=(S[:,::2*K,::2*K,0].unsqueeze(-1)*A0+S[:,::2*K,::2*K,1].unsqueeze(-1)*B0+X0)*Ue[:,::2*K,::2*K,:]
		D=torch.amin(torch.linalg.norm(torch.moveaxis(source,1,2).unsqueeze(1).unsqueeze(1)-Xl.unsqueeze(-1),dim=-2),dim=-1)
		depth[depth==0.]=1.e12
		ind=(torch.argmin(depth,dim=0)[:,:,0])
		Dl=D[ind,torch.arange(80).unsqueeze(-1),torch.arange(40).unsqueeze(0)]
		I=ind.repeat_interleave(2*K,dim=0).repeat_interleave(2*K,dim=1)
		Ar2=wall_im[I,G[I,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),0],G[I,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),1],:].clone()

		depth2=torch.amin(depth,dim=0)
		#print(1000*(time.time()-ttt),'2')

		return Ar2,depth2.repeat_interleave(2,dim=0).repeat_interleave(2,dim=1),Dl.repeat_interleave(2,dim=0).repeat_interleave(2,dim=1)
	
def render_batch_transp(A0,B0,X0,borne,wall_im,Format,source,Icut):
		...
		D0=D[:Icut,:,:]
		depth0=depth[:Icut,:,:,:]
		wall_im0=wall_im[:Icut,:,:,:]
		ind0=(torch.argmin(depth0,dim=0)[:,:,0])
		Dl0=D0[ind0,torch.arange(80).unsqueeze(-1),torch.arange(40).unsqueeze(0)]
		I0=ind0.repeat_interleave(2*K,dim=0).repeat_interleave(2*K,dim=1)
		Ar0=wall_im0[I0,G0[I0,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),0],G0[I0,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),1],:].clone()
		
		G1=G[Icut:,:,:,:]
		D1=D[Icut:,:,:]
		depth1=depth[Icut:,:,:,:]
		wall_im1=wall_im[Icut:,:,:,:]
		ind1=(torch.argmin(depth1,dim=0)[:,:,0])
		#Dl1=D1[ind1,torch.arange(80).unsqueeze(-1),torch.arange(40).unsqueeze(0)]
		I1=ind1.repeat_interleave(2*K,dim=0).repeat_interleave(2*K,dim=1)
		Ar1=wall_im1[I1,G1[I1,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),0],G1[I1,torch.arange(160).unsqueeze(1),torch.arange(80).unsqueeze(0),1],:].clone()
		
		
		filt=torch.unsqueeze(torch.all(Ar1[::2*K,::2*K,:]<=0,dim=2),-1)
		depth0=torch.amin(depth0,dim=0)
		depth2=torch.amin(depth1,dim=0)
		depth2=depth2+1.e8*(filt)
		

		
		d1=(depth2<depth0).repeat_interleave(2,dim=0).repeat_interleave(2,dim=1)
		d0=(depth2>=depth0).repeat_interleave(2,dim=0).repeat_interleave(2,dim=1)

		Ar2=Ar1*d1+Ar0*d0
		#Dl2=Dl0*(depth2>=depth0)[:,:,0]+Dl1*(depth2<depth0)[:,:,0]

		return Ar2,depth0.repeat_interleave(2,dim=0).repeat_interleave(2,dim=1),Dl0.repeat_interleave(2,dim=0).repeat_interleave(2,dim=1)
...
while running==1:
	...
	for i in doors[0:5]:
		...
		if i.norm<6 and (i.door==1 or (CARTE[min(i.door-2,2)]>0 and i.door-2<3) or(10000+code==i.door)):
			...
			if (10000+code==i.door):# REMPLACER i.X par i.X0
				code_show2+=1
				if code_show2>10:
					i.door=1
					code_show2=0
	...
	Lb=[i for i in wall[:50] if (i.test_behind() or (i in h_wall and i.norm<=6))]
	Lb0=[i for i in Lb if i.window==0]
	Lb1=[i for i in Lb if i.window>0]
	batch_C=create_batch(Lb[:30])
	
	
	
	batch_R=render_batch(*batch_C)
	...
	# else:
		# Sky_view=1
		# batch_C1=create_batch(Lb0+Lb1)
		# batch_R=render_batch_transp(*batch_C1,len(Lb0))

	Im=batch_R[0].cpu().numpy()
	depth=batch_R[1].cpu().numpy()
	POS=batch_R[2].cpu().numpy()
	
	

	#print(1000*(time.time()-start),'batch')
	
	# if c%24==0:
		# for i in range(3):
	
			# plt.imshow(batch_R[i,:,:,:].numpy())
			# plt.show()
	
	# to_device()
	# create_batch(wall[0:10])
	# render_batch(wall[])
