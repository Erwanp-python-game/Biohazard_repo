...
def load_level(level_name):
    ...
    f.close()
    time1 = time.perf_counter() * 1000
    print((time1 - time0) / 1000, 'pickle_load')
    ratio = 2
    level_light = [1, 1, 1]
    ...
    for i in sphere:# all radius
        ...
        if wall[-1].sphere>=2:
            ...
            wall[-1].format = 120 * np.array([int(R_*2*pi), int(R_*2*pi)]) / 10
        wall[-1].ID = str(int((xw[0]) // 2 + 50)) + ',' + str(
            int((xw[1]) // 2 + 50))
    ...
    time1 = time.perf_counter() * 1000
    print((time1 - time0) / 1000, 'created_walls')
